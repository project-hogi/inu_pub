...
function getNavsToHtml($tab, $navs, $depth = 100, $curerntDepth = 1, $first = true) {
	$html = ($first ? null : $tab) . "<ul>\r\n";
	
	foreach ( $navs as $nav ) {
		$hasChildren = isset ( $nav ["children"] ) && count ( $nav ["children"] );
		
		$html .= $tab . "\t<li" . (isset ( $nav ["on"] ) && $nav ["on"] ? " class=\"active\"" : null) . ">" . ($hasChildren ? "\r\n" : null);
...
function getPageName($pagePath, $depth = 100) {
	$depth -= 1;
	$sitemap = getSitemap ( $pagePath );
	
	$pathIds = getAllPathIds ( $pagePath );
	$nav = findNavById ( $pathIds, (count ( $pathIds ) > $depth ? $depth : count ( $pathIds )) - 1 );
